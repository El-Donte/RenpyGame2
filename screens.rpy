...
screen quick_menu():
    ...
    if quick_menu:
        ...
        hbox:
            ...
            textbutton _("{icon=icon-skip-back} Назад") action Rollback()
            textbutton _("{icon=icon-compass} История") action ShowMenu('history')
            textbutton _("{icon=icon-fast-forward} Пропуск") action Skip() alternate Skip(fast=True, confirm=True)
            textbutton _("{icon=icon-play-circle} Авто") action Preference("auto-forward", "toggle")
            textbutton _("{icon=icon-save} Сохранить") action ShowMenu('save')
            textbutton _("Q.Save") action QuickSave()
            textbutton _("Q.Load") action QuickLoad()
            textbutton _("{icon=icon-settings} Опции") action ShowMenu('preferences')
...
screen navigation():
    ...
    vbox:
        ...
        if main_menu:
            ...
            textbutton _("{icon=icon-feather} Начать") action Start()
        ...
        textbutton _("{icon=icon-bookmark} Загрузить") action ShowMenu("load")

        textbutton _("{icon=icon-settings} Настройки") action ShowMenu("preferences")
        ...
        if _in_replay:
            ...
            textbutton _("{icon=icon-grid} Главное меню") action MainMenu()

        textbutton _("{icon=icon-info} Об игре") action ShowMenu("about")

        if renpy.variant("pc") or (renpy.variant("web") and not renpy.variant("mobile")):

            ## Help isn't necessary or relevant to mobile devices.
            textbutton _("{icon=icon-help-circle} Помощь") action ShowMenu("help")

        if renpy.variant("pc"):

            ## The quit button is banned on iOS and unnecessary on Android and
            ## Web.
            textbutton _("{icon=icon-x-circle} Выход") action Quit(confirm=not main_menu)
